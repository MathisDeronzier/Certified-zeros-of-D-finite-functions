...
\begin{document}
	...
	Ce chapitre énoncera quelques propriétés des fonctions D-finies, de sorte à donner des notions fondamentales au lecteur non familier avec ce type de fonctions. les démonstration ne seront pas présentés ici, mais sont présentes dans l'article de Stanley d'où elles sont issues.\\
	\\
	...
	\[f(z+t)=\text{exp}\Big(K/T \sum_{k\geq1}\alpha_{k}(z)t^{k} \Big)\]
	
	\[= 1+(K/T)\frac{\sum_{k\geq1}\alpha_{k}(z)t^{k}}{1!}+(K/T)^{2}\frac{\big(\sum_{k\geq1}\alpha_{k}(z)t^{k}\big)^{2}}{2!}+...+(K/T)^{l}\frac{\big(\sum_{k\geq1}\alpha_{k}(z)t^{k}\big)^{l}}{l!}+...\]
	...
	\begin{proposition}
		...
		\[\forall t \in \mathbb{R}_+, \hspace{3mm} m(f,x)(t) \leq |f(x+t)|, \hspace{3mm} m(f,x)(t) \leq |f(x-t)|.\]
		 Si de plus elle est non constante, on pose $\tau=m(f,x)^{-1}(0)$, on peut assurer que $f$ ne s'annule pas sur l'intervalle $]x-\tau,x+\tau[$.
	\end{proposition}
	\vspace{7mm}
	On voit que la partie négative de $m(f,x)$ peut être remplaçée par une série majorante et respecter encore ces propriétés. C'est ce que nous feront concrêtement sur ordinateur.\\
	\\
	...
	de sorte que
	\[r \leq |f'(x_0)^{-1}g(x)|+M^{2}(f,x_0)(r) \]
	C'est à dire que l'hypothèse de Rouché est satisfaite si
	\[\beta(f,x_0)<r-M^{2}(f,x_0)(r)\] 
	Ce sera cette expression qui permettra d'exclure des intervalles. Pour commencer on peut dire que si 
	\[\beta(f,x_0)< \sup_{r \in [0,\mathbb{S}_{f}(x_0)]}\{r-M^{2}(f,x_0)(r)\}\]
	alors, pour tout r tel que l'inégalité  $\beta(f,x_0)<r-M^{2}(f,x_0)(r)$ soit satisfaite, on peut assurer avec le théorème de Rouché qu'il n'existe qu'un seul zéro dans l'intervalle $[x_0-r,x_0+r]$
	...
	\subsection{retour sur les séries majorantes}
	...
	Nous n'irons pas plus loins dans l'explication de l'obtention des bornes, mais la démonstration est détaillée dans l'article de Thomas Grégoire
