...
\begin{document}
	...
	\subsection{Comment vérifier les hypothèse de contraction} 
	...
	On a finalement, pour $z \leq \alpha$
	\[\left\|g'(z)^{-1}\frac{g^{(k)}(z)}{k!}\right\|^{\frac{1}{k-1}}= \frac{\alpha}{1-\alpha z} \left\|\frac{\prod_{i=1}^{K-1}(k+i)}{K!}\right\|^{\frac{1}{k-1}} \tag{*}\]
	Considérons cette expression comme une fonction de x au lieu de k. On a donc un polynôme de degré $K$.
	\[P(x)=\frac{\prod_{i=1}^{K-1}(x+i)}{K!}\]
	...
	\[(x-1)P'(x)-P(x)ln(P(x))=P(x)\Big[\big(\sum_{i=1}^{K-1}\frac{x-1}{x+i}\big)-ln(P(x))\Big]\]
	On majore $\frac{x-1}{x+i}$ par 1 et minore $P(x)$ par $x^{K-1}$ On est sûr que la série est décroissante lorsque 
	\[K-1-ln(x^{K-1})<0 \Longleftrightarrow e^{1}< x\]
	On sait alors que 
	\[\gamma(f,z)=max\Big(\left\|f'(z)^{-1}\frac{f^{(2)}(z)}{2!}\right\|,\left\|f'(z)^{-1}\frac{f^{(3)}(z)}{3!}\right\|^{\frac{1}{2}}\Big) \]
	...
	une autre façon d'exclure des régions de l'espace, maintenant que nous avons des majorations de $\gamma$ est la méthode de Rouché. On introduira la même notation que dans le livre de Dedieu.\\
	\begin{definition} Soient $\mathbb{E}$ et $\mathbb{f}$ des espace de Banach et 
	...
	\end{definition}
		...
		Le théorème suivant permet de minorer ce nombre de séparation à partir du coefficient $\gamma$\\
	...
	On pourra alors exclure de petits intervalle
