...
\begin{document}
	...
	Donc finalement,
	\[m(x)>\frac{\beta(f,x)}{\gamma(f,x) \beta(f,x)+1} \]
	Remarquons que $\gamma$ est une fonction continue donc bornée sur $[a,b]$ appellons $sup\gamma$ son sup sur l'intervalle. On a donc toujous
	\[m(x)>\frac{\beta(f,x)}{\beta(f,x)sup\gamma+1}\]
	Montrons alors que cet algorithme converge bien. On s'inspire de la théorie de Kantorovitch pour démontrer cette assertion.
